...
export class UsernameGenerationService implements BaseUsernameGenerationService {
  ...
  async generateForwarded(options: any): Promise<string> {
    ...
    } else if (o.forwardedService === "pacedemail") {
      if (
        o.forwardedPacedEmailApiToken == null ||
        o.forwardedPacedEmailApiToken === "" ||
        o.forwardedPacedEmailAccount == null ||
        o.forwardedPacedEmailAccount == ""
      ) {
        return null;
      }
      return this.generatePacedEmailAlias(
        o.forwardedPacedEmailApiToken,
        o.forwardedPacedEmailAccount,
        o.website
      );
  ...
  private async generatePacedEmailAlias(
    apiToken: string,
    account: number,
    websiteNote: string
  ): Promise<string> {
    if (apiToken == null || apiToken === "") {
      throw "Invalid Paced Email API token.";
    }
    const requestInit: RequestInit = {
      redirect: "manual",
      cache: "no-store",
    ...
    const url = `https://www.paced.email/api/bitwarden/inboxes?account_id=${account}`;
    requestInit.body = JSON.stringify({
      inbox: {
        description:
    ...
    throw "Unknown Paced Email error occurred.";
  }
