...
type ExpectedAccountType = {
  settings?: { enableAlwaysOnTop?: boolean };
};
...
export class MoveDesktopSettingsMigrator extends Migrator<12, 13> {
  async migrate(helper: MigrationHelper): Promise<void> {
    ...
    async function migrateAccount(userId: string, account: ExpectedAccountType) {
      if (account?.settings?.enableAlwaysOnTop != null) {
        await helper.setToUser(userId, ALWAYS_ON_TOP_KEY, account.settings.enableAlwaysOnTop);
