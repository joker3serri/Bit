...
### `disableFormOnLoading`

The `bitSubmit` directive has the ability to disable the entire form (instead of just the buttons) whenever there
is an async action being processed. This is turned off by default and should be used conservatively and purposefully.
