...
## Icon Button as a Toggle

The icon button can be used to toggle the visibility of another content area. To do so:

1. Create a `buttonType="muted"` icon button. (This is currently the only supported button type for
   use as a toggle button)
2. Set a template reference on the icon button
3. Use the `bitToggledBy` directive on the content area, and pass it the icon button template
   reference
4. Set the `expanded` property on the icon button to init the toggle as either currently expanded or
   currently collapsed

The toggle functionality follows the Disclosure (Show/Hide) pattern for accessibility.
...
<Story of={stories.ToggleIconButton} />
