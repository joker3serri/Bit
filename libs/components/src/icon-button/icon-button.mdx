...
The padding on the `small` size uses .25rem on all sides, while the padding on the `default` size
matches that of the button component.

Both sizes display the icon at 1rem.
...
Always use the `appA11yTitle` element set to a string that describes the action of the icon-button.
This will auto assign the same string to the `title` and `aria-label` attributes.
