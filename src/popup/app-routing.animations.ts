...
const queryShown = query(":enter, :leave", [style({ position: "fixed", width: "100%", height: "100%" })], {
    optional: true,
});
...
const outSlideRight = [queryShown, group([queryTranslateX("enter", -100, 0), queryTranslateX("leave", 0, 100)])];
...
const outSlideDown = [queryShown, group([queryTranslateY("enter", 0, 0), queryTranslateY("leave", 0, 100, 1010)])];
...
const outSlideUp = [queryShown, group([queryTranslateY("enter", 0, 0), queryTranslateY("leave", 0, -100, 1010)])];
...
export function tabsToCiphers(fromState: string, toState: string) {
    ...
    return (
        (fromState.indexOf("ciphers_") === 0 && fromState.indexOf("ciphers_direction=b") === -1) || fromState === "tabs"
    );
}
